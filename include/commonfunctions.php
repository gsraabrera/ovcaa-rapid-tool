...
function checkTableName($shortTName )
{
	...
	if ("ovcaa_rapid_users" == $shortTName )
		return true;
	...
	if ("enrollment_count_by_unit_and_batch" == $shortTName )
		return true;
	if ("grade_submission_monitoring" == $shortTName )
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("public.ovcaa_rapid_users");
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("enrollment_count_by_unit_and_batch");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("grade_submission_monitoring");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("unposted_student_grades");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function GetUserPermissionsStatic( $table )
{
	...
	$extraPerm = "";
	if( $table=="public.ovcaa_rapid_users" )
	{
//	default permissions
...
//	default permissions
	...
	if( $table=="grade_submission_monitoring" )
	{
//	default permissions
		// grant all by default
		return "ADESPI".$extraPerm;
